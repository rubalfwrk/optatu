...
apiRouter.post('/users/adminlogin', function(req, res, next) {   
...
apiRouter.post('/users/register', function(req, res) { 
    ...
    var img = req.body.image;
   // var key = uuid.v4();
    var dat = img.replace(/^data:image\/\w+;base64,/, "");
    ...
    s3.putObject(data, function(err, data){
        ...
        var pro_pic = "https://s3.us-east-2.amazonaws.com/optatu/"+randomString+".jpg";
            ...
            if(req.body.image!=""){
               finalarray['image']=pro_pic;
            }
           console.log(finalarray);
            User.register(new User(finalarray), req.body.password, function(err, user) {
            if(err) {
                res.json({"message":err.message,"error":"1"})
            } else {
            var userdata = {};    
...
apiRouter.post('/users/login', function(req, res, next) {       
    passport.authenticate('local', function(err, user, info) { 
        if (err) {
         return res.json({ success : false, message : 'Invalid credentials'}); // will generate a 500 error
     } 
     if (! user) {             
    ...
    res.json({"message" : "No data Found","error" : 1 });
        ...
        if(user.image){
          ...
          res.json({"message" : "Result Fetched Successfully","error" : 0 ,"data" : user});  
             ...
             console.log("rubal");
            user.image = "https://s3.us-east-2.amazonaws.com/optatu/optatudefault.jpg";
           res.json({"message" : "Result Fetched Successfully","error" : 0 ,"data" : user});
        }
...
apiRouter.post('/changePassword', function(req, res, next) { 
  ...
  apiRouter.post('/users/adminlogin1', function(req, res, next) { 
...
//    console.log(req.body);
    User.findById({'_id': req.body.id}, function(err, user) {
        ...
        var pro_pic = "https://s3.us-east-2.amazonaws.com/optatu/"+key+".jpg";
        //console.log(pro_pic);
        if (err){
            res.send(err);
        }else{
