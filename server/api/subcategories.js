...
module.exports = function(apiRouter,s3,randomString,userupload){
	...
	//subcat by id
        ...
        console.log(req.body.id);
            Subcategory.findById({_id : req.body.id}, function(err, post){
                console.log(post);
                    if (err) res.send(err);
        ...
        Subcategory.findById({'_id': req.body.id}, function(err, sub) {
            
            if (err){
                res.send(err);
            }else{
            sub.category_id = req.body.category_id;               
            sub.image = req.body.category_id;  
            sub.city = req.body.city;  
            sub.location_type = req.body.location_type;
            sub.category = req.body.category;
            sub.category_name = req.body.category_name;
            sub.address = req.body.address;  
            sub.start_date = req.body.start_date;  
            sub.end_date = req.body.end_date;  
            sub.start_time = req.body.start_time;  
            sub.end_time = req.body.end_time;
            sub.save(function(err) {
                if (err){
                    res.send({"error" : 1,"message" : "Unable to edit sub category"});
                }else{
                res.json({"error":0,"message":'Sub category updated successfully','data':sub});
                }
                })
            }
            });
    ...
    /* add sub category kuldeep */
     apiRouter.post('/add_subcategories_admin', function(req, res){
        ...
        var sub = new Subcategory();
            ...
            sub.end_time = req.body.end_time;  
            console.log(sub);
            sub.save(function(err,subcat) {
                console.log("hello");
            if (err){
                res.send({"error" : 1,"message" : "Unable to add Sub category"});
            }else{
                console.log("else");
            res.json({"error":0,"message":'New Sub category added successfully','data':subcat});
        }
        })
